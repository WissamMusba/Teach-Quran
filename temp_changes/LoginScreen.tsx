...
export default function LoginScreen({ navigation }: any) {
   ...
   * FLOW: 1) Reject while a login is already in flight (loading guard) — a
   *          double-tap cannot fire two signInWithEmailAndPassword calls.
   *       2) Client-side guard: empty username or password -> AlertModal
   *          "Please enter both username and password." and return
   *       3) setLoading(true) -> spinner replaces the button label
   *       4) const res = await loginUser(u.trim(), p) — auth.ts:
   *          formatUsernameToEmail (lowercased, non-alphanumerics stripped,
   *          + "@quranmaster.app") then signInWithEmailAndPassword; Firebase
   *          errors mapped to friendly text. loginUser never throws by
   ...
   * CALLED BY: "Login" button onPress
   * AFFECTS: authSlice.user / authSlice.isAuthenticated; navigation stack
   *          (Login -> Dashboard via replace); App.tsx:31-56 side effect
   *          re-runs on isAuthenticated change (initial sync + interval +
   *          AppState listener)
   ...
   *        the stored redux user is mostly cosmetic. Password is sent
  ...
  const handleLogin = useCallback(async () => {
    ...
    try {
      ...
      setAlert({ visible: true, title: 'Login Failed', message: e?.message || 'An unexpected error occurred.' })
    } finally {
      setLoading(false)
    }
  }, [u, p, loading, navigation, dispatch])

  return (
    // KeyboardAvoidingView: iOS does not resize the window for the keyboard, so
    // the centered form (and its Login button) can sit under the keyboard;
