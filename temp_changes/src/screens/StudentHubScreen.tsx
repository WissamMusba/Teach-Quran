 ...
 *       straight from SQLite (getStudentData), refreshed on focus and after every
 *       completed sync, and deep-links into the reader / list screens.
 * FLOW: 1) inline copy of the real src/hooks/useStudentDataRefresh.ts — focus reload
 *       gated while a pull is in flight (a mid-pull reload would show PARTIAL data),
 *       plus a syncing->synced watcher; 2) RESUME resolves lastRead -> page via
 *       getVersePage (script-aware) + juz via JUZ_MAP scan; 3) rows navigate with the
 *       same { surahId, scrollToVerse } / { page } params QuranViewScreen consumes.
...
export default function StudentHubScreen({ navigation }: any) {
  ...
  /**
   * WHAT: Re-reads the student's full data blob (chunks + manifest) from SQLite and
   *       publishes it to Redux — the hub's single source of freshness.
   * CALLS: getStudentData (localDB.ts), dispatch(setStudentData).
   * CALLED BY: useFocusEffect + the sync-status watcher below.
   * AFFECTS: s.student.studentData — replaced with the fresh SQLite snapshot.
   * NOTES: Copy of the real src/hooks/useStudentDataRefresh.ts (absent from temp_changes).
   */
  const reload = useCallback(() => {
    if (!currentStudentId) return;
    getStudentData(currentStudentId).then((d: any) => { if (d) dispatch(setStudentData(d)); }).catch(() => {});
  }, [currentStudentId, dispatch]);

  // Focus reload gated while a pull is in flight (mid-pull = partial data); the
  // syncing->synced watcher below then applies everything in one shot.
  ...
  return (
    <View style={[styles.container, { backgroundColor: bg }]}>
      {/* inline header (ScreenHeader does not exist in temp_changes): back + title + student name */}
