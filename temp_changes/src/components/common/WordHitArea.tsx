 ...
 * USED BY: src/components/quran/MushafPageView.tsx (:17 import, :554-557); src/components/quran/FlowingText.tsx (:17, :88-89); src/components/quran/VerseDisplay.tsx (:17, :69-70)
 */
 ...
 * CALLED BY: MushafPageView.tsx:554-557 → tapFraction=WORD_TAP_FRACTION; onWordPress toggles the mistake highlight;
 *            onLongPress opens the verse menu with pageY; onMeasured feeds handleWordMeasured (the fit-to-line measure dance).
 *            FlowingText.tsx:88 / VerseDisplay.tsx:69 → flowing/ayah modes, same tapFraction.
 ...
 *   - measureInWindow is async; pageX is the finger position, not the word's origin — hence the margin math. The tap is
 *     classified by the RELEASE position: a press that starts in-band but drags out of band (or vice versa) is judged by
 *     where the finger lifts. Deliberate — do not swap to locationX without device testing (Android locationX is
 *     unreliable with transforms/nested views).
 *   - If measureInWindow fails (view detached, returns 0), localX = pageX → classified dead tap: safe fallback.
 */
