 ...
 * FLOW: responder negotiation is deepest-first — touches on buttons or the grip are claimed by that child, so the wrap only becomes responder for frame touches. Grip tap toggles open/docked; EXIT collapses + onExit() (parent exits drawing mode). Drag clamps the toolbar on-screen (open state uses the stricter open clamps so the bar can never be pushed off-screen).
 ...
 * NOTES: position (x/y) is NOT persisted across restarts; `open` is global Redux state, so surah/page changes collapse the toolbar externally (also triggering the selection/palette-reset effect).
 */
const AnnotationToolbar: React.FC<Props> = ({ visible, drawingGestureActive, onUndo, onRedo, onClear, onExit, canUndo, canRedo, onActivateDraw }) => {
  ...
  // Selection + palette reset — every time the toolbar collapses (EXIT, grip tap, page change) the highlight clears and the palette closes, so reopening looks brand new (no "pen selected" ghost even though drawingSlice.activeTool still holds 'pen'; no palette popping open unexpectedly after an external collapse).
  useEffect(() => { if (!open) { setSelectedTool(''); setPal(false); } }, [open]);
  const [[x, y], setPos] = useState([MARGIN, initY]);
  const dragStart = useRef({ x: 0, y: 0, px: 0, py: 0 });
  const posRef = useRef({ x: MARGIN, y: initY });

  // Clamps — when open the toolbar can never leave the screen (clampXOpen/clampYOpen keep the FULL bar reachable: the bar extends BAR_W+GAP to the right of the grip and V_EXTRA above/below its center, so the open bounds must not reuse the collapsed half-off-screen peeks); when collapsed, half-off-screen docking positions are allowed; clampY guards status-bar and bottom edges.
  ...
  const clampYOpen = (v: number) => Math.max(sbHeight, Math.min(v, height - BOT - V_EXTRA - DOCK_PEEK));
  ...
  const onTouchMove = useCallback((e: any) => {
    ...
    const nx = open ? clampXOpen(newPx) : clampX(newPx);
    const ny = open ? clampYOpen(dragStart.current.py + dy) : clampY(dragStart.current.py + dy);
    setPos([nx, ny]);
  ...
  const reclampOnExpand = useCallback((px: number, py: number) => {
    return { x: clampXOpen(px), y: clampYOpen(py) };
   ...
   * NOTES: the snapped position is KEPT as-is (undocking happens on the next grip TAP, which collapses + exits — there is no restore-position path, so no pre-dock snapshot is needed).
   */
