...
// WHAT: Maps textStyle keys to registered font family names: saleem->PDMSSaleemQuranFont, indopak->PDMSSaleemQuranFont (same Indopak glyph font as saleem), uthmani->KFGQPCUthmanicScriptHAFS, alqalam->AlQalamQuranMajeed, lateef->Lateef-Regular, harmattan->Harmattan-Regular, amiri->Amiri-Regular, scheherazade->ScheherazadeNew-Regular.
// NOTES: 'indopak' is a legacy key (QuranViewScreen's indopakFonts list and VerseDisplay/FlowingText's isIndopakStyle both match it — indopak pages/text), so it must resolve to the Indopak font rather than silently falling back to uthmani. Unknown keys still fall back to uthmani.
export const ARABIC_FONTS: Record<string, string> = {
  saleem: 'PDMSSaleemQuranFont',
  indopak: 'PDMSSaleemQuranFont',
...
// NOTES: Never crashes on undefined/unknown style (falls back to uthmani); a family that is not yet loaded renders as the system fallback font rather than crashing.
