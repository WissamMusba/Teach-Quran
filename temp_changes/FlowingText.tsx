 ...
 *      plain whitespace + cleanQuranWord (no pageData/location wordPos in this mode). The word
 *      array is NOT filtered: stored highlight wordIndex values index into this exact array.
 ...
 *     what's visible.
 *   - Malformed-verse guard: a null verse (or one missing verseNumber) renders nothing instead of
 *     crashing the whole scroll; missing Arabic text renders an empty row.
 *   - memo() is only as good as prop identities: QuranViewScreen's handlers re-create when
 *     handleWordFlow's deps change (canvasData), so the memo mainly blocks re-renders when only
 *     unrelated state churns.
 */
const FlowingText = ({ verses, highlights, onWordPress, onVerseLongPress, onBookmarkToggle, showTranslation, fontSize, bookmarks, notes, flashingVerse, readingMarkVerse, onDeadTap }: any) => {
  ...
  // below is dead code. FONT_SIZES[fontSize] falls back to 'medium' so a corrupted/undefined
  // fontSize tier cannot produce a NaN size.
  ...
  return (
    <View style={styles.container}>
      {verses.map((verse: any) => {
        // Malformed-row guard: a null verse or one without a verse number must not reach the
        // field reads below.
...
// memo export — see NOTES: only effective when the parent passes stable prop identities.
