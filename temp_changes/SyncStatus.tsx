 ...
 * WHAT: 'syncing' -> blue 'Syncing...'; 'offline' -> red `Offline (${pendingChanges})`; 'synced' -> green 'Synced';
 *       otherwise grey 'Idle' — but 'Idle (n)' when unsynced annotations exist (pendingChanges > 0) while no
 *       sync run has happened yet (fresh login window before App.tsx's first runSync resolves).
 ...
 * FIX (idle desync): plain 'Idle' hid the pending count when status was still 'idle' but pendingChanges > 0 —
 *          the pill disagreed with the header "Sync (n)" badge until the first sync run dispatched.
 */
export default function SyncStatus() {
  ...
  else if (status === 'idle' && pendingChanges > 0) { text = `Idle (${pendingChanges})`; }
