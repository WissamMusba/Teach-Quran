 ...
 *        FIX (fade race): the fade-out completion callback is guarded on `finished` — RN invokes the old
 *        animation's callback with finished:false when a NEW animation starts on the same Animated.Value,
 *        so a rapid 'synced' -> 'syncing' transition used to run setVisible(false) mid-fade-in and hide the
 *        pill while still syncing. An interrupted fade-out no longer hides it.
 */
const SyncIndicator = () => {
  const { status } = useSelector((s: any) => s.sync);
