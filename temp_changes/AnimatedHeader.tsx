 ...
 *       7) The parallel composite is kept in a ref and stopped on re-trigger AND on unmount (effect cleanup) —
 *          the JS-driven timing loop never keeps ticking after the header is gone.
 ...
 *          Visibility is purely prop-driven — swipe gestures live in QuranViewScreen (PanGestureHandler + edge Pressables);
 *          src/hooks/useHeaderSwipe.ts is a dead file, never imported.
 */
const AnimatedHeader: React.FC<Props> = (p) => {
  ...
  useEffect(() => {
    ...
    anim.current.start();
    // Cleanup: stop the composite on unmount so no JS-driver frames tick after the header is gone.
