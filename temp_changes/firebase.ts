 ...
 * FLOW: 1) Read auth().currentUser ONCE into a local. 2) Return uid or null.
 ...
 *        uid is never undefined in a path. Single-read fix: reading
 *        auth().currentUser twice (guard + uid) could observe a sign-out
 *        between the two reads and crash on null.uid (TOCTOU).
 */
