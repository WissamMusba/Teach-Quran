 ...
 * FLOW: 1) `activeButtons` = `buttons` when it has entries, otherwise a single 'OK' button built from onClose (or nothing);
 *       2) buttons render with destructive -> red / cancel -> dark / last plain -> primary #00d4aa;
 *       3) onPress runs btn.onPress?.() then onClose?.() — always closes.
 ...
 *        An EMPTY `buttons` array (truthy) now falls back to the default OK button instead of
 *        rendering zero buttons — an empty dialog would otherwise be undismissable on iOS.
 */
        ...
        <View style={[styles.box, { backgroundColor: bg, borderColor: border }]}>
          ...
          <View style={styles.row}>
            {activeButtons.map((btn, i) => {
              ...
              const isLast = i === activeButtons.length - 1;
              return (
                <TouchableOpacity
                  key={i}
                  style={[
