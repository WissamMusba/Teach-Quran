...
export default function QuranViewScreen({ navigation, route }: any) {
  ...
  // warmedPagesRef keys: `${textStyle}|${keyW}|${pageNum}` — a page counts as warmed per font +
  // layout width (the cache-key dimensions), so a font or split-width change re-measures cleanly.
  ...
  const hiddenTick = useCallback(() => {
    ...
    // Nearest-first, ahead-biased (ahead 12 / behind 10 — inside the prefetch data window, so
    // page JSON is already in pageCache and the hidden slot renders the moment it mounts).
    const c = currentPageNum;
    const total = pageNumbers.length;
    const keyW = Math.round(splitOn ? pageW : winW);
    const cands: number[] = [];
    for (let d = 1; d <= 12; d++) {
      if (c + d <= total) cands.push(c + d);
      if (d <= 10 && c - d >= 1) cands.push(c - d);
    }
    const next = cands.find(p =>
      !warmedPagesRef.current.has(`${textStyle}|${keyW}|${p}`) &&
      pageCache[p]?.lines?.length > 0
    );
    if (!next) return;
  ...
  }, [hiddenFocus, isDrawing, isCapturing, drawingGestureActive, readingMode, settledPage, currentPageNum, pageNumbers.length, splitOn, pageW, winW, textStyle, pageCache]);
  ...
  }, [hiddenWarmPage, splitOn, pageW, textStyle]);
  ...
  }, [currentPageNum, settledPage, pageCache, hiddenWarmPage, isDrawing, isCapturing, drawingGestureActive, hiddenFocus, readingMode]);
   ...
   *   ensurePageLoaded promise, 3s cap), warms the layout rows for the target
   ...
   * FLOW: stamp -> race ensurePageLoaded(pg) against a 3s null timer -> if
        ...
        <GestureHandlerRootView style={{ flex: 1 }}><PanGestureHandler onHandlerStateChange={onSwipe} activeOffsetY={[-15, 15]} activeOffsetX={[-25, 25]} enabled={!isDrawing && readingMode !== 'page'}>
          <View style={{ flex: 1, position: 'relative' }}>
            ...
            {/* v76 — background hidden pre-measure slot: ONE off-screen page measuring at a time
                 ...
                 page box so the measured geometry is identical). Its layout row lands in SQLite
                 before the user swipes there, so arrival renders instantly as a cache hit. */}
            {readingMode === 'page' && hiddenWarmPage && pageCache[hiddenWarmPage] && (
              <View pointerEvents="none" style={{ position: 'absolute', left: -10000, top: 0, bottom: 0, width: splitOn ? pageW : winW, overflow: 'hidden', opacity: 0.999 }}>
                <View style={{ flex: 1, marginHorizontal: winW >= 600 ? 10 : 6, marginTop: 24, marginBottom: 28 }}>
                  <MushafPageView hideFrame pageNum={hiddenWarmPage} pageWidth={splitOn ? pageW : winW} headerVisible={isHeaderVisible} surahNames={surahNames}
