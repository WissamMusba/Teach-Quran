    ...
    <ScrollView style={[styles.container, { backgroundColor: nightMode ? '#121212' : '#FFFFFF' }]}>
      <View style={styles.section}>
        ...
        <View style={styles.styleContainer}>
           {['saleem', 'uthmani', 'mequran', 'lateef', 'harmattan', 'amiri', 'scheherazade', 'noto'].map((style) => (
            <TouchableOpacity key={style} style={[styles.styleBtn, textStyle === style && styles.activeMode]} onPress={() => dispatch(setTextStyle(style as any))}>
              <Text style={textStyle === style ? styles.activeText : styles.inactiveText}>{style === 'saleem' ? 'INDOPAK' : style === 'mequran' ? 'ME QURAN' : style === 'lateef' ? 'LATEEF (WAQF)' : style.toUpperCase()}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
