...
export const saveManifestLocal = async (studentId: string, manifest: any, serverTs = 0) => {
  ...
  await addToSyncQueue(studentId, '_manifest');
};
...
export const addToSyncQueue = async (studentId: string, canvasKey: string) => {
  const db = getDB();
  await db.executeSql(
    `INSERT INTO sync_queue (studentId, canvasKey, retryCount) VALUES (?, ?, 0) 
     ON CONFLICT(studentId, canvasKey) DO UPDATE SET retryCount=0`,
    [studentId, canvasKey]
  );
};
