...
export interface DrawingCanvasHandle {
  ...
  redo: () => void;
  clear: () => void;
  flush: () => void;   // fire the pending debounced save NOW (used by the app-state background flush)
}
...
const DrawingCanvas = forwardRef<DrawingCanvasHandle, Props>(({ visible, initialPaths = [], onSave, onStateChange, onGestureStart, onGestureEnd }, ref) => {
  ...
  const lastPenPointRef = useRef<string | null>(null);
  ...
  /** handleFlush — same semantics as the unmount flush, callable on demand (background push). */
  const handleFlush = () => {
    if (saveTimerRef.current) {
      clearTimeout(saveTimerRef.current);
      saveTimerRef.current = null;
